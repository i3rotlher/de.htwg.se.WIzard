...
class GamestateTest extends AnyWordSpec {
  "Gamestate" when {
    "initialized with these players" should {
      ...
      "and when handing out the cards in round 5 they should all be diffrent" in {
        game = game.generate_Hands(5)
        (game.players(0).hand diff game.players(1).hand).size shouldBe(5)
        (game.players(1).hand diff game.players(2).hand).size shouldBe(5)
      }
      "and generate a trumpcard out of the remaining cards" in {
        game = game.generate_Hands(5)
        game.players(0).hand.contains(game.get_Trump_card(game.players)) shouldBe(false)
      ...
      "When a player played a card should have the remaining cards" in {
        game = game.generate_Hands(5)
        val toplay = game.players(0).hand(0)
        game = game.playCard(toplay, 0)
        game.players(0).hand.contains(toplay) shouldBe(false)
      }
    }
  }
}