  ...
  "A controller" when {
    ...
    "beeing initialized" should {
      ...
      "when called set player amount should have the player list with the given size" in {
        ...
        controller1.game.getPlayers.size shouldBe 3
        val controller2 = controller
        controller2.set_player_amount(Some(4))
        controller2.game.getPlayers.size shouldBe 4
        val controller3 = controller
        controller3.set_player_amount(Some(5))
        controller3.game.getPlayers.size shouldBe 5
        val controller4 = controller
        controller3.set_player_amount(Some(6))
        controller3.game.getPlayers.size shouldBe 6
      ...
      "when given a player name should set the getPlayers name" in {
        ...
        controller2.game.getPlayers(0).getName shouldBe "Torsten"
      }
      "when given a player and undo is invoked should reset the name" in {
        ...
        controller2.game.getPlayers(controller2.game.getActive_player_idx).getName shouldBe "unknown"
      }
      "when given a player, and undo has been alled and redo is beeing called should have the name again" in {
        ...
        controller2.game.getPlayers(controller2.game.getActive_player_idx-1).getName shouldBe "Torsten"
      }
      "when given a wished color should set the trumpcard" in {
        ...
        controller3.game.getTrump_card shouldBe new Card_with_value(1, "red")
      }
      "when get mini winner is called should return the winner idx" in {
        val controller4 = new Controller(controller.game, null)
        controller4.game.getPlayers.indexOf(controller4.get_mini_winner()) shouldBe 0
      }
    }
    ...
    "already beeing initialized with players" should {
      ...
      "after start_round has been called" in {
        ...
        controller1.game.getRound_number shouldBe 0
      }
      "after set_guess has been called" in {
        ...
        controller2_1.game.getGame_table(0).guessed_tricks(0) shouldBe(2)
      }
      "after set_guess has been called by the last player" in {
        ...
        controller2.set_guess(3).getActive_player_idx shouldBe 0
      }
    }
