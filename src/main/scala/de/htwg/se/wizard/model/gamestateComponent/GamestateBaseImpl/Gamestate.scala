...
case class Gamestate (players: List[PlayerInterface] = List(), game_table: List[Round] = List(), round_number: Int = 0,
  ...
  def generate_Hands(round_number: Int, players: List[PlayerInterface]): GamestateInterface = {
    ...
    for (i <- players.indices) {
      ...
      n_playerList = n_playerList.updated(i, Player(players(i).getName, tmpCard_Tuple._1))
    }
  ...
  def set_Trump_card(player: List[PlayerInterface], round_nr: Int): Gamestate = {
  ...
  def playCard(played_card: Card, active_player: PlayerInterface): Gamestate = {
  ...
  override def reset_player(): GamestateInterface= {
