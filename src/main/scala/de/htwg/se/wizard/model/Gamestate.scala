...
case class Gamestate(players: List[Player] = List(), game_table: List[Round] = List(), round_number: Int = 0,
  ...
  //def set_guesses(guessed_tricks: List[Int]): Gamestate = copy(players, game_table.appended(Round(guessed_tricks)), round_number = round_number + 1)
  ...
  def create_players(players: List[Player]): Gamestate = Gamestate(players, made_tricks = List.fill(players.size) {0}, mini_starter_idx = 0, round_number = 0, game_table = game_table.appended(Round(List.fill(players.size){0})))
